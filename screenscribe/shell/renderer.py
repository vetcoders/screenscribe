...
def _assert_no_unfilled_shell_slots(rendered: str) -> None:
    unresolved_slots = re.findall(r"\{slot:[^}]+\}", rendered)
    if unresolved_slots:
        raise ValueError(f"Unresolved shell slots: {', '.join(unresolved_slots)}")
...
def render_surface(config: SurfaceConfig, context: Mapping[str, Any]) -> str:
    ...
    replacements = {
        ...
        "{slot:head}": _head(config, context),
        "{slot:window_actions}": _render_optional_partial(config.window_actions, context),
        "{slot:tabs}": _render_tabs(config, context),
        "{slot:header_right}": _render_header_cells(config, context),
        "{slot:main_panels}": _render_partials(config.main_panels, context),
        "{slot:sidebar_panels}": _render_partials(config.sidebar_panels, context),
        "{slot:sidebar_footer}": _render_optional_partial(config.footer, context),
        "{slot:modals}": _render_partials(config.modals, context),
        "{slot:scripts}": _script_blocks(config, context),
        "{slot:document_footer}": _document_footer(config),
    }
    rendered = shell
    for marker, value in replacements.items():
        rendered = rendered.replace(marker, value)
    _assert_no_unfilled_shell_slots(rendered)
    return rendered
